...
@testset "RegularGrid" begin
    @testset "Check that the stepsizes are being computed correctly" begin
        lb = [2.0, 3.0, 5.0]
        ub = [5.0, 5.3, 13.2]
        res = 50
    ...
    @testset "Test that the grid is constructed correctly" begin
        lb = [2.7, 5.3, 10.0]
        ub = [10.0, 7.7, 14.4]
        res = 50
    ...
    @testset "Test that we can get the parameters correctly" begin
        lb = [2.7, 5.3, 10.0, 4.4]
        ub = [10.0, 7.7, 14.4, -57.4]
        res = [50, 32, 10, 100]
        ug = ProfileLikelihood.RegularGrid(lb, ub, res)
...
@testset "FusedRegularGrid" begin
    @testset "Getting the resolution tuples" begin
    ...
    @testset "Varying resolutions" begin
    ...
    @testset "Storing the original resolutions" begin
        lb = [2.0, 3.0, 1.0, 5.0]
        ub = [15.0, 13.0, 27.0, 10.0]
        centre = [7.3, 8.3, 2.3, 7.5]
