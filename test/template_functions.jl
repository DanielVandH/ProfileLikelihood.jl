...
function LinearExponentialODEAutoDiff()
    Random.seed!(2992999)
    λ = -0.5
    y₀ = 15.0
    σ = 0.1
    ...
    @inline function loglik(θ, data)
        local yᵒ, n, t
        yᵒ, n, t, T = data
        ode_problem = ODEProblem(ode_fnc, θ[3], (0.0, T), θ[1])
        sol = solve(ode_problem, Tsit5(); saveat = t, verbose = false)
        return gaussian_loglikelihood(yᵒ, sol.u, θ[2], n)
    end
    θ₀ = [-1.0, 0.5, 19.73]
    prob = LikelihoodProblem(loglik, 3;
        data=(yᵒ, n, t, T), θ₀, lb=[-10.0, 1e-6, 0.5], ub=[10.0, 10.0, 25.0],
        names=[L"\lambda", L"\sigma", L"y_0"],
        syms=[:λ, :σ, :y₀])
    return prob, loglik, (λ, σ, y₀), yᵒ, n
end
