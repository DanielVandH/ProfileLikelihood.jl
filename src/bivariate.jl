...
function construct_profile_grids(n::NTuple{N,NTuple{2,I}}, sol, lower_bounds, upper_bounds, resolutions) where {N,I}
    grids = Dict{NTuple{2,Int},FusedRegularGrid{
        2,
    ...
    for (n₁, n₂) in n
        n₁idx, n₂idx = variable_index(sol, n₁), variable_index(sol, n₂)
        if any(isinf, lower_bounds[[n₁idx, n₂idx]]) || any(isinf, upper_bounds[[n₁idx, n₂idx]])
            throw("The provided parameter bounds for $n₁ and $n₂ must be finite.")
        end
        grids[(n₁idx, n₂idx)] = FusedRegularGrid(
            [lower_bounds[n₁idx], lower_bounds[n₂idx]],
            [upper_bounds[n₁idx], upper_bounds[n₂idx]],
            [sol[n₁idx], sol[n₂idx]],
            max(res[n₁idx][1], res[n₁idx][2], res[n₂idx][1], res[n₂idx][2]);
