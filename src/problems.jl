...
"""
@inline data(prob::AbstractLikelihoodProblem) = prob.prob.p
...
"""
    lower_bounds(prob::OptimizationProblem)
    lower_bounds(prob::AbstractLikelihoodProblem) 
    upper_bounds(prob::OptimizationProblem)
    upper_bounds(prob::AbstractLikelihoodProblem) 
    bounds(prob::OptimizationProblem, i)
    bounds(prob::OptimizationProblem)
    bounds(prob::AbstractLikelihoodProblem, i)
    bounds(prob::AbstractLikelihoodProblem)
...
@doc (@doc bounds) @inline bounds(prob::OptimizationProblem, i) = (prob.lb[i], prob.ub[i])
...
################################################################################
"""
    LikelihoodProblem{ST,iip,F,θType,P,B,LC,UC,S,K,θ₀Type,ℓ<:Function} <: AbstractLikelihoodProblem
